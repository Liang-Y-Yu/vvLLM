...
from src.backend.enums import t5_type


def get_loaders(model_name, reward_type, llama_type=None, load_gptq='', load_awq='', load_exllama=False, config=None,
                rope_scaling=None, max_seq_len=None, model_name_exllama_if_no_config=''):
    # NOTE: Some models need specific new prompt_type
    # E.g. t5_xxl_true_nli_mixture has input format: "premise: PREMISE_TEXT hypothesis: HYPOTHESIS_TEXT".)
    if load_exllama:
        from src.backend.llm_exllama import CustomExLlamaTokenizer, CustomExLlamaGenerator
        ...
        tokenizer = CustomExLlamaTokenizer(tokenizer_path)  # create tokenizer from tokenizer model file
        tokenizer.model_max_length = exconfig.max_seq_len

        cache = ExLlamaCache(model)  # create cache for inference
        generator = CustomExLlamaGenerator(model, tokenizer, cache)  # create generator
