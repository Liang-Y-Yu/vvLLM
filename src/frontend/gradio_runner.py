...
import pandas as pd
import requests
from src.frontend.iterators import TimeoutIterator

from src.frontend.css import get_css
from src.frontend.prompt_form import make_chatbots
...
from src.backend.enums import DocumentSubset, no_model_str, no_lora_str, no_server_str, LangChainAction, LangChainMode, \
    DocumentChoice, langchain_modes_intrinsic, LangChainTypes, langchain_modes_non_db, gr_to_lg
from src.frontend.gradio_themes import SoftTheme, get_simple_title, \
    get_dark_js, wrap_js_to_lambda, \
    spacing_xsm, radius_xsm, text_xsm
from src.backend.prompter import prompt_type_to_model_name, prompt_types_strings, inv_prompt_type_to_model_lower, non_hf_types, \
    get_prompt
from src.backend.utils import flatten_list, zip_data, s3up, clear_torch_cache, get_torch_allocated, system_info_print, \
...
from src.gen import get_model, languages_covered, evaluate, score_qa, inputs_kwargs_list, \
    get_max_max_new_tokens, get_minmax_top_k_docs, history_to_context, langchain_actions, langchain_agents_list
from src.backend.evaluate_params import eval_func_param_names, no_default_param_names, eval_func_param_names_defaults, \
...
def go_gradio(**kwargs):
    ...
    if kwargs['langchain_mode'] != 'Disabled':
        from src.middleware.gpt_langchain import file_types, have_arxiv
    else:
        have_arxiv = False
        file_types = []
    ...
    demo = gr.Blocks(theme=theme, css=css_code, title="GPT", analytics_enabled=False)
    ...
    def get_request_state(requests_state1, request, db1s):
        ...
        if requests:
            ...
            if not requests_state1.get('username', '') and hasattr(request, 'username'):
                from src.middleware.gpt_langchain import get_username_direct
    ...
    def user_state_setup(db1s, requests_state1, request: gr.Request, *args):
        ...
        from src.middleware.gpt_langchain import set_userid
    ...
    with demo:
        ...
        def get_df_langchain_mode_paths(selection_docs_state1, db1s, dbs1=None):
            ...
            if langchain_mode_types:
                ...
                from src.middleware.gpt_langchain import get_persist_directory, load_embed
        ...
        def add_langchain_mode(db1s, selection_docs_state1, requests_state1, langchain_mode1, y,
            ...
            if valid and not user_path:
                ...
                from src.middleware.gpt_langchain import length_db1
        ...
        def remove_langchain_mode(db1s, selection_docs_state1, requests_state1,
            ...
            if langchain_mode2 in langchain_modes or langchain_mode2 in langchain_mode_paths or langchain_mode2 in db1s:
                if can_purge and purge:
                    # remove source files
                    from src.middleware.gpt_langchain import get_sources, del_from_db
                    ...
                    for fil in source_list:
                        if os.path.isfile(fil):
                            ...
                            remove(fil)
                    # remove db directory
                    from src.middleware.gpt_langchain import get_persist_directory
        ...
        def clear_embeddings(langchain_mode1, db1s):
            ...
            if db_type == 'chroma' and langchain_mode1 not in ['LLM', 'Disabled', None, '']:
                from src.middleware.gpt_langchain import clear_embedding, length_db1
...
def show_doc(db1s, selection_docs_state1, requests_state1,
    ...
    if db_type1 == 'chroma':
        ...
        langchain_mode_paths = selection_docs_state1['langchain_mode_paths']
        langchain_mode_types = selection_docs_state1['langchain_mode_types']
        from src.middleware.gpt_langchain import set_userid, get_any_db, get_docs_and_meta
...
def update_user_db_gr(file, db1s, selection_docs_state1, requests_state1,
    ...
    from src.middleware.gpt_langchain import update_user_db
...
def get_sources_gr(db1s, selection_docs_state1, requests_state1, langchain_mode, dbs=None, docs_state0=None,
                   ...
                   api=False,
                   n_jobs=-1):
    from src.middleware.gpt_langchain import get_sources
    sources_file, source_list, num_chunks, db = \
...
def get_source_files_given_langchain_mode_gr(db1s, selection_docs_state1, requests_state1,
                                             ...
                                             get_userid_auth=None,
                                             n_jobs=-1):
    from src.middleware.gpt_langchain import get_source_files_given_langchain_mode
...
def update_and_get_source_files_given_langchain_mode_gr(db1s,
                                                        ...
                                                        jq_schema0=None):
    from src.middleware.gpt_langchain import update_and_get_source_files_given_langchain_mode
...
def set_dbid_gr(db1):
    from src.middleware.gpt_langchain import set_dbid
    return set_dbid(db1)


def set_userid_direct_gr(db1s, userid, username):
    from src.middleware.gpt_langchain import set_userid_direct
