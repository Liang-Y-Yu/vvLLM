...
from src.backend.evaluate_params import eval_func_param_names, no_default_param_names, input_args_list
from src.backend.enums import DocumentSubset, LangChainMode, no_lora_str, model_token_mapping, no_model_str, \
...
from src.backend.loaders import get_loaders
from src.backend.utils import set_seed, clear_torch_cache, NullContext, wrapped_partial, EThread, get_githash, \
    import_matplotlib, get_device, makedirs, get_kwargs, start_faulthandler, get_hf_server, FakeTokenizer, \
    have_langchain, set_openai, cuda_vis_check, gpt_fire, lg_to_gr, str_to_list, str_to_dict
...
from src.backend.prompter import Prompter, inv_prompt_type_to_model_lower, non_hf_types, PromptType, get_prompt, generate_prompt
from src.backend.stopping import get_stopping
...
def main(
    ...
    :param load_awq: to load model with AWQ, put model_basename here, e.g. model
    ...
    :param inference_server: Consume base_model as type of model at this address
                             ...
                             e.g. python generate.py --inference_server="http://192.168.1.46:6112" --base_model=TheBloke/llama-2-7b
    ...
    :param llamacpp_dict:
           ...
           ... etc. anything that could be passed to llama.cpp models
    ...
    :param prepare_offline_level:
           ...
           0 : no prep
           1: prepare just with exact same setup as passed to CLI and ensure all artifacts for alone added to ~/.cache/
           2: prepare all inference servers so inference servers can use the ~/.cache/
    ...
    :param langchain_modes: dbs to generate at launch to be ready for LLM
           Apart from additional user-defined collections, can include ['UserData', 'MyData']
    ...
    if have_langchain and langchain_mode is None:
        ...
        if allow_upload_to_user_data and langchain_mode_paths['UserData']:
            if verbose:
    ...
    git_hash = get_githash() if os.getenv('GET_GITHASH') else "GET_GITHASH"
    locals_dict = locals()
    ...
    if langchain_mode != "Disabled":
        ...
        from src.middleware.gpt_langchain import prep_langchain, get_persist_directory
        dbs = {}
    ...
    if cli:
        ...
        from src.backend.eval import run_eval
        ...
        from src.frontend.gradio_runner import go_gradio
        ...
        if enable_captions:
            if pre_load_caption_model:
                ...
                caption_loader = CustomImageCaptionLoader(caption_gpu=caption_gpu).load_model()
            else:
        ...
        if pre_load_embedding_model and langchain_mode != 'Disabled' and not use_openai_embedding:
            from src.middleware.gpt_langchain import get_embedding
...
def get_client_from_inference_server(inference_server, base_model=None, raise_connection_exception=False):
    ...
    from src.frontend.grclient import GradioClient
    gr_client = None
    hf_client = None
    if headers is None:
        try:
...
def get_model(
    ...
    model_loader, tokenizer_loader, conditional_type = (
        get_loaders(model_name=base_model, reward_type=reward_type, llama_type=llama_type,
                    load_gptq=load_gptq, load_awq=load_awq, load_exllama=load_exllama, config=config,
    ...
    if base_model in non_hf_types:
        from src.middleware.gpt4all_llm import get_model_tokenizer_gpt4all
...
def evaluate(
    ...
    from src.backend.text_pipeline import CustomTextGenerationPipeline
    instruction, num_prompt_tokens1 = CustomTextGenerationPipeline.limit_prompt(instruction, tokenizer)
    context, num_prompt_tokens2 = CustomTextGenerationPipeline.limit_prompt(context, tokenizer)
    iinput, num_prompt_tokens3 = CustomTextGenerationPipeline.limit_prompt(iinput, tokenizer)
    ...
    from src.middleware.gpt_langchain import get_any_db
    ...
    if do_langchain_path:
        ...
        sources = ''
        response = ''
        from src.middleware.gpt_langchain import run_qa_db
