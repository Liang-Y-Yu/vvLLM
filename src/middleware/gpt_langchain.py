...
from src.backend.enums import DocumentSubset, no_lora_str, model_token_mapping, source_prefix, source_postfix, non_query_commands, \
    LangChainAction, LangChainMode, DocumentChoice, LangChainTypes, font_size, head_acc, super_source_prefix, \
    super_source_postfix, langchain_modes_intrinsic, get_langchain_prompts
from src.backend.evaluate_params import gen_hyper, gen_hyper0
from src.gen import get_model
from src.backend.prompter import non_hf_types, PromptType, Prompter
from src.backend.utils import wrapped_partial, EThread, import_matplotlib, sanitize_filename, makedirs, get_url, flatten_list, \
...
from src.middleware.utils_langchain import StreamingGradioCallbackHandler
...
from src.backend.chromamig import ChromaMig
...
class GPTOpenAI(OpenAI):
    ...
    def _generate(
        ...
        for prompti, prompt in enumerate(prompts):
            prompt, num_prompt_tokens = CustomTextGenerationPipeline.limit_prompt(prompt, self.tokenizer)
...
class GPTChatOpenAI(ChatOpenAI):
    @classmethod
    def _all_required_field_names(cls) -> Set:
...
def get_llm(use_openai_model=False,
    ...
    # currently all case return prompt + new text, but could change
    only_new_text = False
    ...
    if inference_server.startswith('replicate'):
        ...
        from src.middleware.gpt4all_llm import get_llm_gpt4all
        ...
        from src.backend.llm_exllama import Exllama
        ...
        from src.backend.text_pipeline import CustomTextGenerationPipeline
        pipe = CustomTextGenerationPipeline(model=model, use_prompter=True,
                                            prompter=prompter,
                                            context=context,
                                            iinput=iinput,
...
def file_to_doc(file,
    ...
    if is_url:
        ...
        if have_doctr and enable_doctr:
            ...
            if model_loaders['doctr'] is not None and not isinstance(model_loaders['doctr'], (str, bool)):
                ...
                from src.middleware.image_doctr import CustomOCRLoader
                model_loaders['doctr'] = CustomOCRLoader()
        ...
        if enable_captions:
            ...
            if model_loaders['caption'] is not None and not isinstance(model_loaders['caption'], (str, bool)):
                ...
                model_loaders['caption'] = CustomImageCaptionLoader(caption_gpu=model_loaders['caption'] == 'gpu',
                                                                    blip_model=captions_model,
                                                                    blip_processor=captions_model)
            model_loaders['caption'].set_image_paths([file])
            docs1c = model_loaders['caption'].load()
            docs1c = [x for x in docs1c if x.page_content]
            add_meta(docs1c, file, parser='CustomImageCaptionLoader: %s' % captions_model)
            # caption didn't set source, so fix-up meta
            for doci in docs1c:
        ...
        if enable_pix2struct:
            ...
            if model_loaders['pix2struct'] is not None and not isinstance(model_loaders['pix2struct'], (str, bool)):
                ...
                from src.middleware.image_pix2struct import Pix2StructLoader
                model_loaders['pix2struct'] = Pix2StructLoader()
            model_loaders['pix2struct'].set_image_paths([file])
            docs1c = model_loaders['pix2struct'].load()
            docs1c = [x for x in docs1c if x.page_content]
            add_meta(docs1c, file, parser='CustomPix2StructLoader: %s' % model_loaders['pix2struct'])
            # caption didn't set source, so fix-up meta
            for doci in docs1c:
                doci.metadata['source'] = doci.metadata.get('image_path', file)
                doci.metadata['hashid'] = hash_file(doci.metadata['source'])
    ...
    # doc1 = GCSFileLoader(project_name, bucket, blob).load()
        ...
        if len(doc1) == 0 or enable_pdf_doctr:
            ...
            doc1a = [x for x in doc1a if x.page_content]
            add_meta(doc1a, file, parser='CustomOCRLoader: %s' % 'DocTR')
            handled |= len(doc1a) > 0
            # caption didn't set source, so fix-up meta
            for doci in doc1a:
                doci.metadata['source'] = doci.metadata.get('document_path', file)
        ...
        if llm_mode:
            ...
            if top_k_docs == -1 or auto_reduce_chunks:
                ...
                if top_k_docs == -1:
                    ...
                    doc_content, new_tokens0 = CustomTextGenerationPipeline.limit_prompt(doc_content,
                                                                                         tokenizer,
